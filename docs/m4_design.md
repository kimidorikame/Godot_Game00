...
骨子時点の「就寝時セーブのみ（Result画面で『翌日へ』押下時のみ）」方針を変更した。
Result画面から離脱する操作（「翌日へ」「タイトルへ」の両方）でオートセーブする。

- 保存される `day` は従来通り「次にプレイすべき日（N+1）」で統一。
  「翌日へ」「タイトルへ」どちらの経路でも保存内容は同一になる。
  ロードすると N+1日目の昼（DestinationSelect）から再開される
- Day7（エンディング直行）はセーブしない方針は維持。
  この判定は `result.gd` の共通関数 `_finalize_day_and_save()` 内の
  `day==7` 判定に一元化した（`_on_next_day_pressed()` / `_on_to_title_pressed()`
  側には day==7 の分岐を持たせない）
- Day7のResultでは「タイトルへ」ボタンを `hide()` し、エンディングをスキップして
  タイトルに戻れてしまう抜け道を塞いだ（案③）
- ×ボタンでの強制終了はセーブ対象外（Godotの終了処理には介入しない）
- `SaveManager.SAVE_PATH` → `AUTO_SAVE_PATH` にリネーム。将来の複数スロット＋
  任意タイミング手動セーブ拡張への布石。今回は単一スロットのまま、
  スロット機構自体は未実装（将来スコープ）

why: 「N日目終了→タイトル→CONTINUE」がN日目の頭に戻る違和感（progress_logの
M5検討事項）の解消。「タイトルへ」でも「翌日へ」と同じ状態を保存することで、
どちらの経路で離脱してもCONTINUEの復帰先が一致するようにした。

この変更は、困窮脱出動線（クズ食材ベース＝鍋の日またぎ持ち越し）が前提とする
「Result到達時点の状態保存」の土台を兼ねる。将来、鍋の volume/attrs をセーブ対象に
加える際は `_finalize_day_and_save()` に乗せる想定。
